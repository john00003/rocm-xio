...
vm_build() {
  ...
  if [ ! -d "$TOOLING/.git" ]; then
    ...
    git clone --branch "$QEMU_MINIMAL_BRANCH" "$QEMU_MINIMAL_REMOTE" "$TOOLING" \
  ...
  if [ ! -d "$ANSIBLE_SRC/.git" ]; then
    ...
    git clone --branch "$ANSIBLE_BRANCH" "$ANSIBLE_REMOTE" "$ANSIBLE_SRC" \
  ...
  ansible-galaxy collection install "$ANSIBLE_SRC" --force >>"$LOG_DIR/gen-vm.log" 2>&1 || true
